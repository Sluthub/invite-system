      ...
      <div class="flex flex-col space-y-2">
        <Transition name="fade" mode="out-in">
          <div v-if="contributors" class="flex flex-col space-y-2">
            <div
              class="space-y-2 divide-y divide-gray-200 rounded dark:divide-gray-700"
            >
              ...
              >
                ...
                <span class="text-gray-400 text-center">{{
                  __(
                    "We currently do not have access to Open Collective. If you are making payments, please cancel them."
                  )
                }}</span>
              </div>
            </div>
...
export default defineComponent({
  ...
  async created() {
    const response = await this.$axios
      .get<any, { data: OpenCollectiveResponse }>(
        "https://opencollective.com/wizarr/members/all.json?limit=30",
        {
          method: "GET",
        }
      )
      ...
      .reduce(
        (acc, contributor) =>
          ((existing) =>
            existing
              ? acc.map((item) =>
                  item.profile === contributor.profile ? { ...item } : item
                )
              : [...acc, contributor])(
            acc.find((item) => item.profile === contributor.profile)
          ),
        [] as OpenCollectiveResponse
      )
      .sort(
        (a, b) =>
    ...
    console.log(
      "%cTop Contributors",
      "color: #2196f3; font-size: 1.5rem; font-weight: bold;"
    );
    console.log(
      "%cThank you for supporting the Screening Room!",
    ...
    Object.entries(sorted).forEach(([index, contributor]) => {
      console.log(
        `%c${parseInt(index) + 1}. ${contributor.name} - ${this.formatCurrency(
