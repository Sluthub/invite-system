  ...
  <div>
    ...
    >
      ...
      <FormKit
        ...
        :help="__('Display Name for your media servers')"
        name="server_name"
        placeholder="The Screening Room"
      ...
      <FormKit
        type="inputButton"
        :label="__('Media Server Address')"
        :help="__('Server IP or Address of your media server')"
        name="server_url"
        validation="required"
        prefix-icon="fas text-gray-400 fa-arrow-up-right-from-square"
        placeholder="https://watch.therealsarahwalker.com"
        @button="detectServer"
        autocomplete="url"
        :classes="{ prefixIcon: 'hidden' }"
      >
        {{ __("Detect Server") }}
      </FormKit>
      ...
      <FormKit
        type="text"
        :label="__('Media Server Override')"
        :help="
          __(
            'Optional if your server address does not match your external address'
          )
        "
        name="server_url_override"
        placeholder="https://watch.therealsarahwalker.com"
        validation="trim|url"
        autocomplete="url"
      />
      ...
      <FormKit
        ...
        :help="__('Detected Media Server')"
        name="server_type"
        placeholder="Choose a server"
        prefix-icon="fas fa-server"
      ...
      <FormKit
        ...
        :help="__('API key for your media server')"
        name="server_api_key"
        prefix-icon="fas text-gray-400 fa-key"
        placeholder="••••••••"
    ...
    <div class="flex flex-col sm:flex-row">
      <div class="flex flex-grow justify-end sm:justify-start space-x-2 mt-2">
        ...
        <VTooltip>
          ...
          <FormKit
            type="button"
            :disabled="true"
            v-if="!serverForm.server_type || !setup"
            prefix-icon="fas fa-server"
            data-theme="secondary"
            @click="scanServers"
          >
      ...
      <div class="flex flex-grow justify-end sm:justify-end space-x-2 mt-2">
        ...
        <FormKit
          type="button"
          v-if="!verified"
          :disabled="!serverForm.server_type || !serverForm.server_api_key"
        ...
        <FormKit
          ...
          v-if="verified && !saved"
          :disabled="!serverForm.server_type || !serverForm.server_api_key"
          @click="$formkit.submit('serverForm')"
        >
          {{ __("Save Connection") }}
        </FormKit>
...
export default defineComponent({
  ...
  methods: {
    ...
    syncSettings() {
      this.$axios
        .get("/api/settings", { disableErrorToast: true })
        .then((response) => {
          if (response.data)
            this.serverForm = {
              ...this.serverForm,
    ...
    async detectServer() {
      if (!this.serverForm.server_url)
        return this.$toast.error(this.__("Please enter a server URL."));
      ...
      this.$toast.info(
        this.__("Detected %{server_type} server!", {
          server_type:
            response.data.server_type.slice(0, 1).toUpperCase() +
            response.data.server_type.slice(1),
        })
      );
    },
    async verifyConnection() {
      if (!this.serverForm.server_url || !this.serverForm.server_api_key)
        return this.$toast.error(
          this.__("Please enter a server URL and API key.")
        );
    ...
    async saveConnection() {
      ...
      let server_url_override = this.serverForm.server_url_override
        ? this.serverForm.server_url_override.trim().replace(/\/$/, "")
        : "";

      // Automatically set server_url_override to https://app.plex.tv if it contains plex.tv, otherwise leave it as is
      server_url_override =
        server_url_override && server_url_override.includes("plex.tv")
          ? "https://app.plex.tv"
          : server_url_override;
      ...
      if (!this.setup) {
        const confirm = await this.$modal.confirmModal(
          ...
          this.__(
            "Are you sure you want to save this connection, this will reset your Wizarr instance, which may lead to data loss."
          )
        );
        if (!confirm) return;
      }
