...
export default defineComponent({
  ...
  async mounted() {
    ...
    const response = await this.$axios.get("/api/setup/status").catch(() => {
    ...
    if (response.data.setup_stage) {
      if (
        ...
        this.currentView =
          this.views.findIndex((view) => view.name === "complete") + 1;
      }
      ...
      if (
        ...
        this.currentView =
          this.views.findIndex((view) => view.name === "settings") + 1;
      }
    }
