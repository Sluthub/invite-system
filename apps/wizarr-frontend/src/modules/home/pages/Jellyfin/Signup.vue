<template>
    <div ref="jellyForm">
        <FormKit type="form" id="jellyfinForm" v-model="form" @submit="submit()" submit-label="Create Account" :submit-attrs="{ wrapperClass: 'flex justify-end' }">
            <FormKit type="text" label="Username" name="username" placeholder="kinky_whore" validation="required:trim|alphanumeric" required autocomplete="off" />
            <FormKit type="email" label="Email" name="email" placeholder="clara@sluthub.is" validation="required:trim|email" required autocomplete="email" />
